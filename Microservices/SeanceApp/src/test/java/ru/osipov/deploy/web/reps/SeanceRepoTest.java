...
public class SeanceRepoTest {
    ...
    @Test
    void findByDate() {
        List<Seance> r = sRep.findAllByDate(LocalDate.parse("2019-12-05"));//CHECK
        assert r != null;
        assert r.size() > 0;
    }
    ...
    void findByDateBetween(){
        List<Seance> r = sRep.findAllByDateBetween(LocalDate.parse("2019-12-01"),LocalDate.parse("2019-12-05"));
        assert r != null;
        assert r.size() == 2;
    }
    ...
    void findByDateBefore(){
        List<Seance> r = sRep.findAllByDateBefore(LocalDate.parse("2019-12-01"));
        assert r != null;
        assert r.size() == 2;
    }
